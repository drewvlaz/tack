...
│   ├── boardItemTags.ts addTags, removeTags, listBoardTags (bulk, normalized to lowercase)
...
CORS is locked to localhost dev origins (`5173`/`5174`) in `index.ts` with `credentials: true` so the session cookie can ride along. Adding production: extend the allowlist with the deployed frontend origin — `credentials: true` requires an explicit origin, never `*`. The allowlist predicate lives in `lib/origins.ts` and is shared with the CSRF middleware below.

**CSRF.** Because the session cookie is `SameSite=None` on staging/prod (cross-site frontend↔worker), a Hono middleware in `index.ts` enforces a request-side Origin check on every non-GET/HEAD/OPTIONS request: if `Origin` is present and not in the allowlist, the request is rejected with `403 {error:"forbidden_origin"}`. CORS is the response-side defense (browser-enforced); this is the request-side defense (server-enforced) — the cookie only "works" when both agree, so a CORS misconfig can't silently widen the attack surface. Absent `Origin` (curl, server-to-server) is allowed through: no browser means no automatic cookie attachment, hence no CSRF.

Auth is email + password with sessions stored in D1 (`sessions` table) and an opaque session id sent as the `tack_sess` cookie (HttpOnly, 30-day TTL). Cookie SameSite/Secure/Partitioned attrs are driven by `ENVIRONMENT`: dev uses `SameSite=Lax` without `Secure` (localhost is same-site, HTTP); staging/prod use `SameSite=None; Secure; Partitioned` because the frontend (`*.pages.dev`) and worker (`*.workers.dev`) are cross-site. `None` is required so browsers attach the cookie on credentialed fetch; `Partitioned` (CHIPS) is required so privacy Chromium forks (Helium, Brave) store it under the Pages origin instead of treating it as a blocked third-party cookie. Stock Chrome still accepts the unpartitioned cookie; Firefox already partitions automatically. Hono routes at `/api/auth/{signup,login,logout,me}` (`routes/auth.ts`); tRPC reads the cookie in `createContext` and exposes `ctx.userId` to procedures. Passwords are PBKDF2-SHA256 / 100k iterations (Workers caps PBKDF2 at 100k — OWASP 2023's 600k isn't reachable on this runtime, no compat flag to bypass), stored as a self-describing PHC string (`pbkdf2$100000$<salt>$<hash>`). The iteration count lives in the PHC string, so existing hashes keep verifying if the constant is later bumped. Signup is gated by the `INVITE_EMAILS` allowlist — open signup is off.
